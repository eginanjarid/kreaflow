...
const TYPE_CONFIG: Record<string, { color: string; bg: string; label: string; icon: string; href: string }> = {
  riset:    { color: '#1a73e8', bg: 'rgba(26,115,232,0.08)',  label: 'Riset',    icon: 'search', href: '/sprints' },
    ...
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '32px 20px' }}>
      ...
      <div className="kf-page-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        {unread > 0 && (
          <button onClick={markAllRead} disabled={marking}
            style={{ background: 'transparent', border: '1px solid #e5eaf2', borderRadius: 8, padding: '7px 14px', color: '#6b7280', fontSize: '0.78rem', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {[
          ...
          <button key={f.key} onClick={() => setFilter(f.key)}
            style={{ padding: '6px 14px', borderRadius: 20, border: `1px solid ${filter === f.key ? '#1a73e8' : '#e5eaf2'}`, background: filter === f.key ? 'rgba(26,115,232,0.10)' : 'transparent', color: filter === f.key ? '#1a73e8' : '#6b7280', fontSize: '0.78rem', cursor: 'pointer', fontWeight: filter === f.key ? 700 : 400, transition: 'all 0.15s' }}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: '#6b7280' }}>
          <div style={{ marginBottom: 12, color: '#6b7280' }}><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg></div>
          <div style={{ fontWeight: 600, color: '#6b7280', marginBottom: 6 }}>Tidak ada notifikasi</div>
      ...
      {dayOrder.map(day => (
        <div key={day} style={{ marginBottom: 28 }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>{day}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {groups[day].map(n => {
              const cfg = TYPE_CONFIG[n.type] || { color: '#6b7280', bg: 'rgba(148,163,184,0.1)', label: n.type, icon: 'pin', href: '/sprints' }
              return (
                <div key={n.id}
                  style={{ background: n.is_read ? '#f9fafb' : '#fff', border: n.is_read ? '1px solid #f3f4f6' : '1px solid #e5e7eb', borderRadius: 10, padding: '12px 14px', display: 'flex', gap: 12, alignItems: 'flex-start', transition: 'border-color 0.2s' }}>
                  {/* Icon */}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '0.68rem', color: '#6b7280' }}>{relativeTime(n.created_at)}</span>
                      <Link href={cfg.href}
                      ...
                      {!n.is_read && (
                        <button onClick={() => markRead(n.id)}
                          style={{ fontSize: '0.68rem', color: '#6b7280', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>
                          Tandai dibaca
                        </button>
                      )}
                      <button onClick={() => deleteNotif(n.id)}
                        style={{ fontSize: '0.68rem', color: '#6b7280', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, marginLeft: 'auto' }}>
                        ✕
                      </button>
                    </div>
                  </div>
