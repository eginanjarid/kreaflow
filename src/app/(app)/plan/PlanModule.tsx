...
function SprintBanner({ tasks, productName }: { tasks: TaskSnap[]; productName: string }) {
  ...
  return (
    <div style={{ background: 'rgba(124,58,237,0.06)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: 10, padding: '12px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#7C3AED', letterSpacing: '0.05em' }}>⚡ SPRINT AKTIF — {productName}</span>
        <span style={{ fontSize: '0.7rem', color: '#64748b' }}>{done}/{total} selesai · {pct}%</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
        {sprintTasks.map(t => {
          ...
          return (
            <span key={t.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: '0.68rem', padding: '2px 7px', borderRadius: 4, background: '#1a1a1a', border: '1px solid #2a2a2a', color: '#94a3b8' }}>
              <span style={{ color: dotColor }}>{dot}</span>
              <span>{stepName}</span>
              {dateStr && <span style={{ color: '#475569' }}>{dateStr}</span>}
            </span>
          )
        })}
      </div>
...
Ingat: naskah harus terasa seperti teman yang excited share temuan bagus, bukan sales pitch. Gunakan bahasa sehari-hari Indonesia yang relate untuk ${affForm.target_audiens}.`
  ...
  const STATUS_COLOR: Record<string, string> = { Planning: '#93c5fd', Active: '#86efac', Completed: '#A78BFA', Cancelled: '#475569' }

  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#f1f5f9', marginBottom: 6 }}>Plan</h1>
        <p style={{ color: '#64748b', fontSize: '0.9rem' }}>Strategi platform dan rencana campaign konten kamu</p>
      </div>
      ...
      {tab === 'naskah' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {isAffiliate && (
            <div style={{ display: 'flex', gap: 0, background: '#111', border: '1px solid #2a2a2a', borderRadius: 10, overflow: 'hidden', alignSelf: 'flex-start' }}>
              {[{ id: 'creator', label: '🎬 Creator' }, { id: 'affiliate', label: '🔗 Affiliator' }].map(m => (
                <button key={m.id} type="button" onClick={() => setNaskahMode(m.id as 'creator' | 'affiliate')}
                  style={{ padding: '9px 22px', border: 'none', background: naskahMode === m.id ? 'linear-gradient(135deg, #7C3AED, #A78BFA)' : 'transparent', color: naskahMode === m.id ? '#fff' : '#64748b', fontSize: '0.85rem', fontWeight: naskahMode === m.id ? 700 : 400, cursor: 'pointer', transition: 'all 0.15s' }}>
                  {m.label}
                </button>
              ))}
            </div>
          ...
          {naskahMode === 'creator' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              {/* Form kiri */}
              <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 12, padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <div style={{ fontWeight: 600, color: '#e2e8f0', marginBottom: 4 }}>Konfigurasi Naskah</div>
                  <div style={{ fontSize: '0.78rem', color: '#64748b' }}>Isi detail konten → Generate → paste hasil AI di kanan</div>
                </div>

                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600 }}>Platform</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {['TikTok', 'Instagram', 'YouTube', 'Facebook', 'LinkedIn'].map(pl => (
                      <button key={pl} type="button" onClick={() => setNF('platform', pl)}
                        style={{ padding: '6px 12px', borderRadius: 7, border: `1px solid ${naskahForm.platform === pl ? '#7C3AED' : '#2a2a2a'}`, background: naskahForm.platform === pl ? 'rgba(124,58,237,0.15)' : '#1a1a1a', color: naskahForm.platform === pl ? '#A78BFA' : '#64748b', fontSize: '0.78rem', cursor: 'pointer', fontWeight: naskahForm.platform === pl ? 600 : 400 }}>
                        {pl}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600 }}>Format Konten</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600 }}>Pillar / Tema Konten</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600 }}>Hook Angle</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {['Shock/Surprised', 'Problem-first', 'Result-first', 'Story', 'Question', 'Warning', 'Comparison', 'Social proof'].map(h => (
                      <button key={h} type="button" onClick={() => setNF('hook_angle', naskahForm.hook_angle === h ? '' : h)}
                        style={{ padding: '5px 10px', borderRadius: 6, border: `1px solid ${naskahForm.hook_angle === h ? '#7C3AED' : '#2a2a2a'}`, background: naskahForm.hook_angle === h ? 'rgba(124,58,237,0.12)' : '#1a1a1a', color: naskahForm.hook_angle === h ? '#A78BFA' : '#64748b', fontSize: '0.75rem', cursor: 'pointer' }}>
                        {h}
                      </button>
                    ))}
                  </div>
                ...
                {isAffiliate && products.filter(p => p.tipe_produk === 'Affiliate').length > 0 && (
                  <div>
                    <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600 }}>Produk Affiliate <span style={{ color: '#475569', fontWeight: 400 }}>(opsional)</span></label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600 }}>Konteks Tambahan <span style={{ color: '#475569', fontWeight: 400 }}>(opsional)</span></label>
                ...
                {brandProfile?.niche && (
                  <div style={{ background: '#0d1117', border: '1px solid #1f1f1f', borderRadius: 8, padding: '10px 14px', fontSize: '0.72rem', color: '#475569', lineHeight: 1.7 }}>
                    ...
                    <div>Niche: <span style={{ color: '#64748b' }}>{brandProfile.niche}</span></div>
                    {brandProfile.tone_of_voice && <div>Tone: <span style={{ color: '#64748b' }}>{brandProfile.tone_of_voice}</span></div>}
                    {brandProfile.target_audiens && <div>Audiens: <span style={{ color: '#64748b' }}>{brandProfile.target_audiens}</span></div>}
                  </div>
                )}

                <button type="button" onClick={() => setAiModal({ prompt: buildNaskahPrompt() })}
                  style={{ background: 'linear-gradient(135deg, #7C3AED, #A78BFA)', border: 'none', borderRadius: 10, padding: '12px 20px', color: '#fff', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  ✨ Generate Naskah dengan AI
                </button>
              </div>

              {/* Output kanan */}
              <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 12, padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontWeight: 600, color: '#e2e8f0', marginBottom: 2 }}>Hasil Naskah</div>
                    <div style={{ fontSize: '0.78rem', color: '#64748b' }}>Paste hasil dari AI di sini</div>
                  </div>
                  {generatedNaskah && (
                    <button type="button" onClick={saveToLibrary}
                      style={{ background: savedToLibrary ? 'rgba(52,211,153,0.15)' : 'rgba(124,58,237,0.12)', border: `1px solid ${savedToLibrary ? '#34d399' : '#7C3AED'}`, borderRadius: 8, padding: '7px 14px', color: savedToLibrary ? '#34d399' : '#A78BFA', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}>
          ...
          {naskahMode === 'affiliate' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 760 }}>
              ...
              <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 16, overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid #1f1f1f' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 4, height: 22, borderRadius: 2, background: '#ec4899' }} />
                    <span style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '1.05rem' }}>Data Produk</span>
                  </div>
                  <span style={{ fontSize: '0.68rem', fontWeight: 700, color: '#475569', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 5, padding: '3px 10px', letterSpacing: '0.08em' }}>STEP 01</span>
                </div>
                <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {/* Pilih dari katalog */}
                  {products.length > 0 ? (
                    <div>
                      <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Pilih dari Katalog <span style={{ color: '#475569', fontWeight: 400, textTransform: 'none' }}>(auto-isi deskripsi)</span></label>
                  ...
                  {/* Deskripsi produk */}
                  <div>
                    <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Deskripsi Produk (Input)</label>
                    <textarea
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Hasil Analisis USP <span style={{ color: '#475569', fontWeight: 400, textTransform: 'none' }}>(paste dari AI)</span></label>
                    <textarea
              ...
              <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 16, overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid #1f1f1f' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '1.05rem' }}>Konfigurasi</span>
                  </div>
                  <span style={{ fontSize: '0.68rem', fontWeight: 700, color: '#475569', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 5, padding: '3px 10px', letterSpacing: '0.08em' }}>STEP 02</span>
                </div>
                <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: 16 }}>

                  {/* Formula Copywriting */}
                  <div>
                    <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Formula Copywriting</label>
                  ...
                  {(() => {
                    ...
                    return (
                      <div>
                        <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Niche Produk</label>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                      {(() => {
                        ...
                        return <>
                          <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Target Audiens</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Gender</label>
                  ...
                  {/* Target Durasi */}
                  <div>
                    <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Target Durasi</label>
                  ...
                  {(() => {
                    ...
                    return (
                      <div>
                        <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Style Call to Action</label>
                  ...
                  {(() => {
                    ...
                    return (
                      <div>
                        <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Fokus Konversi (Trigger)</label>
                  ...
                  {(() => {
                    ...
                    return (
                      <div>
                        <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Visual Hook Strategy</label>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    ...
                    <div>
                      {(() => {
                        ...
                        return <>
                          <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Format Konten</label>
                  ...
                  {/* Konteks tambahan */}
                  <div>
                    <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Konteks Tambahan <span style={{ color: '#475569', fontWeight: 400, textTransform: 'none' }}>(opsional)</span></label>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Jumlah Varian Naskah</label>
                    ...
                    <div style={{ fontSize: '0.72rem', color: '#475569', marginTop: 4 }}>Tiap varian punya hook, angle, dan story yang berbeda. Rekomendasi: 2-3.</div>
                  </div>
              ...
              {/* Output hasil naskah */}
              <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 16, padding: '22px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontWeight: 700, color: '#f1f5f9', marginBottom: 2 }}>Hasil Naskah</div>
                    <div style={{ fontSize: '0.78rem', color: '#64748b' }}>Paste hasil dari AI di sini, lalu simpan ke Library</div>
                  </div>
                  {affNaskah && (
              ...
              {affNaskah.trim() && (() => {
                ...
                return (
                  <div style={{ background: '#111', border: '1px solid #1e3a2f', borderRadius: 16, overflow: 'hidden' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid #1f1f1f', background: 'rgba(52,211,153,0.03)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 4, height: 22, borderRadius: 2, background: '#34d399' }} />
                        <span style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '1.05rem' }}>Alur Produksi</span>
                      </div>
                      <span style={{ fontSize: '0.68rem', fontWeight: 700, color: '#475569', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 5, padding: '3px 10px', letterSpacing: '0.08em' }}>STEP 03</span>
                    </div>
                    <div style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                      <div style={{ fontSize: '0.78rem', color: '#64748b', marginBottom: 6 }}>Centang setiap step yang sudah selesai — pantau progress konten ini di Library.</div>
                          ...
                          <div key={step.id} onClick={() => !step.autoCheck && toggleProdStep(step.id)}
                            ...
                            <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? '#34d399' : '#2a2a2a', transition: 'background 0.2s' }}>
                              {done ? (
                                ...
                                <span style={{ fontSize: '0.65rem', color: '#475569', fontWeight: 700 }}>{i + 1}</span>
                              )}
                            </div>
                            ...
                            <div style={{ flex: 1 }}>
                              <div style={{ fontWeight: 600, color: done ? '#34d399' : '#94a3b8', fontSize: '0.875rem' }}>
                                {step.label}
                                {step.optional && <span style={{ fontSize: '0.68rem', color: '#475569', fontWeight: 400, marginLeft: 6 }}>(opsional)</span>}
                              </div>
                              <div style={{ fontSize: '0.72rem', color: '#475569' }}>{step.desc}</div>
                            </div>
                      ...
                      <div style={{ marginTop: 8, display: 'flex', gap: 10 }}>
                        {affSavedToLibrary ? (
                          <div style={{ flex: 1, background: 'rgba(52,211,153,0.08)', border: '1px solid #34d39930', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <a href="/library" style={{ fontSize: '0.78rem', color: '#64748b', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
                              Lihat di Library
      ...
      {tab === 'platforms' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {platforms.map(p => (
            <div key={p.platform} style={{ background: '#111', border: `1px solid ${p.is_active ? 'rgba(124,58,237,0.3)' : '#2a2a2a'}`, borderRadius: 12, overflow: 'hidden' }}>
              {/* Platform Header */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: p.is_active ? '1px solid #1f1f1f' : 'none' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  <span style={{ fontSize: '0.8rem', color: '#64748b' }}>{p.is_active ? 'Nonaktifkan' : 'Aktifkan'}</span>
                  <div onClick={() => setPlatformField(p.platform, 'is_active', !p.is_active)}
                    style={{ width: 36, height: 20, borderRadius: 10, background: p.is_active ? '#7C3AED' : '#2a2a2a', position: 'relative', cursor: 'pointer', transition: 'background 0.2s' }}>
              ...
              {p.is_active && (
                <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Tujuan Platform</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Target Followers</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Target Reach/bulan</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Posting/minggu</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Tipe Konten Prioritas</label>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.78rem', color: '#94a3b8', marginBottom: 8, fontWeight: 500 }}>Waktu Terbaik Posting</label>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {WAKTU.map(t => (
                        <button key={t} type="button" onClick={() => toggleWaktu(p.platform, t)}
                          style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.75rem', fontWeight: 500, border: p.waktu_terbaik?.includes(t) ? '1px solid #7C3AED' : '1px solid #2a2a2a', background: p.waktu_terbaik?.includes(t) ? 'rgba(124,58,237,0.15)' : '#1a1a1a', color: p.waktu_terbaik?.includes(t) ? '#A78BFA' : '#475569', cursor: 'pointer' }}>
                          {t}
                        </button>
                      ))}
                    </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <button type="button" onClick={() => savePlatform(p.platform)} disabled={saving === p.platform}
                      style={{ background: savedPlatform === p.platform ? '#166534' : 'linear-gradient(135deg, #7C3AED, #A78BFA)', border: 'none', borderRadius: 8, padding: '9px 20px', color: '#fff', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', transition: 'background 0.2s' }}>
      ...
      {tab === 'campaigns' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
            <button onClick={openAddCampaign} style={{ background: 'linear-gradient(135deg, #7C3AED, #A78BFA)', border: 'none', borderRadius: 10, padding: '10px 20px', color: '#fff', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}>
              + Tambah Campaign
            </button>
          </div>
          {campaigns.length === 0 ? (
            <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 12, padding: 48, textAlign: 'center', color: '#64748b' }}>
              ...
              <div style={{ fontWeight: 600, color: '#94a3b8', marginBottom: 6 }}>Belum ada campaign</div>
              <div style={{ fontSize: '0.85rem', marginBottom: 20 }}>Rencanakan campaign promo, kolaborasi, atau event khusus</div>
              <button onClick={openAddCampaign} style={{ background: 'linear-gradient(135deg, #7C3AED, #A78BFA)', border: 'none', borderRadius: 8, padding: '10px 20px', color: '#fff', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                        <span style={{ fontWeight: 600, color: '#e2e8f0', fontSize: '0.9rem' }}>{c.nama}</span>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        {(c.platforms || []).map(p => <span key={p} style={{ fontSize: '0.68rem', padding: '1px 6px', borderRadius: 3, color: '#7C3AED', background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.2)' }}>{p}</span>)}
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={() => openEditCampaign(c)} style={{ background: 'rgba(124,58,237,0.1)', border: '1px solid #7C3AED', borderRadius: 7, padding: '5px 10px', color: '#A78BFA', fontSize: '0.75rem', cursor: 'pointer' }}>Edit</button>
                      <button onClick={() => deleteCampaign(c.id!)} style={{ background: 'transparent', border: '1px solid #2a2a2a', borderRadius: 7, padding: '5px 8px', color: '#64748b', fontSize: '0.75rem', cursor: 'pointer' }}>🗑</button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {modal?.open && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}>
          <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 16, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ padding: '18px 24px', borderBottom: '1px solid #1f1f1f', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#f1f5f9' }}>{modal.campaign.id ? 'Edit Campaign' : 'Tambah Campaign'}</h2>
            ...
            <form onSubmit={saveCampaign} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Nama Campaign *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Selesai</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Tujuan</label>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Budget (Rp)</label>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {PLATFORMS.map(p => (
                    <button key={p} type="button" onClick={() => togglePlatform(p)}
                      style={{ padding: '5px 12px', borderRadius: 20, fontSize: '0.78rem', fontWeight: 500, border: modal.campaign.platforms.includes(p) ? '1px solid #7C3AED' : '1px solid #2a2a2a', background: modal.campaign.platforms.includes(p) ? 'rgba(124,58,237,0.15)' : '#1a1a1a', color: modal.campaign.platforms.includes(p) ? '#A78BFA' : '#64748b', cursor: 'pointer' }}>
                      {p}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button type="submit" disabled={savingCampaign} style={{ background: savingCampaign ? '#5B21B6' : 'linear-gradient(135deg, #7C3AED, #A78BFA)', border: 'none', borderRadius: 10, padding: '10px 24px', color: '#fff', fontSize: '0.875rem', fontWeight: 600, cursor: savingCampaign ? 'not-allowed' : 'pointer' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} onClick={e => { if (e.target === e.currentTarget) { setAffAiModal(null); setAffPromptCopied(false) } }}>
          <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 20, width: '100%', maxWidth: 480, padding: 32, display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '1.15rem', marginBottom: 6 }}>{affAiModal.label || '⚗️ Compile Scripts'}</div>
                <div style={{ fontSize: '0.82rem', color: '#64748b' }}>Pilih AI — prompt langsung terisi, paste hasilnya di kolom output</div>
              </div>
              <button type="button" onClick={() => { setAffAiModal(null); setAffPromptCopied(false) }} style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', color: '#64748b', fontSize: '1rem', cursor: 'pointer', padding: '4px 8px', borderRadius: 6 }}>✕</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
      ...
      {aiModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} onClick={e => { if (e.target === e.currentTarget) { setAiModal(null); setPromptCopied(false) } }}>
          <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 20, width: '100%', maxWidth: 480, padding: 32, display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '1.15rem', marginBottom: 6 }}>✨ Generate Naskah</div>
                <div style={{ fontSize: '0.82rem', color: '#64748b' }}>Pilih AI — prompt langsung terisi, paste hasilnya di kolom kanan</div>
              </div>
              <button type="button" onClick={() => { setAiModal(null); setPromptCopied(false) }} style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', color: '#64748b', fontSize: '1rem', cursor: 'pointer', padding: '4px 8px', borderRadius: 6 }}>✕</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
