  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#f1f5f9', marginBottom: 6 }}>Budget</h1>
          <p style={{ color: '#64748b', fontSize: '0.9rem' }}>Tracking pemasukan dan pengeluaran konten kamu</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 28 }}>
        ...
        <div style={{ background: '#111', border: `1px solid ${saldo >= 0 ? 'rgba(134,239,172,0.2)' : 'rgba(248,113,113,0.2)'}`, borderRadius: 12, padding: '18px 20px' }}>
          <div style={{ fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Saldo Bersih</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 700, color: saldo >= 0 ? '#A78BFA' : '#f87171' }}>{saldo < 0 ? '-' : ''}{formatRp(saldo)}</div>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          {['', ...TIPE].map(t => (
            <button key={t} onClick={() => setFilterTipe(t)}
              style={{ padding: '5px 14px', borderRadius: 20, fontSize: '0.78rem', fontWeight: 500, border: filterTipe === t ? '1px solid #7C3AED' : '1px solid #2a2a2a', background: filterTipe === t ? 'rgba(124,58,237,0.15)' : '#1a1a1a', color: filterTipe === t ? '#A78BFA' : '#64748b', cursor: 'pointer' }}>
              {t || 'Semua'}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 12, padding: 48, textAlign: 'center', color: '#64748b' }}>
          ...
          <div style={{ fontWeight: 600, color: '#94a3b8', marginBottom: 6 }}>Belum ada transaksi</div>
        ...
        <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 12, overflow: 'hidden' }}>
          {filtered.map((t, i) => (
            <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderBottom: i < filtered.length - 1 ? '1px solid #1f1f1f' : 'none' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 500, color: '#e2e8f0', fontSize: '0.875rem', marginBottom: 2 }}>{t.deskripsi || t.kategori}</div>
                <div style={{ fontSize: '0.72rem', color: '#475569' }}>{t.tanggal} · {t.kategori}</div>
              </div>
              ...
              <button onClick={() => openEdit(t)} style={{ background: 'transparent', border: '1px solid #2a2a2a', borderRadius: 6, padding: '4px 8px', color: '#64748b', fontSize: '0.72rem', cursor: 'pointer' }}>Edit</button>
              <button onClick={() => deleteTx(t.id!)} style={{ background: 'transparent', border: 'none', color: '#475569', cursor: 'pointer', fontSize: '0.85rem' }}>🗑</button>
            </div>
          ))}
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}>
          <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 16, width: '100%', maxWidth: 460 }}>
            <div style={{ padding: '18px 24px', borderBottom: '1px solid #1f1f1f', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#f1f5f9' }}>{modal.tx.id ? 'Edit Transaksi' : 'Catat Transaksi'}</h2>
            ...
            <form onSubmit={handleSave} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Tipe</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Tanggal</label>
                  <input type="date" style={fieldStyle()} value={modal.tx.tanggal} onChange={e => setField('tanggal', e.target.value)} required />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Deskripsi *</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: 6, fontWeight: 500 }}>Jumlah (Rp) *</label>
