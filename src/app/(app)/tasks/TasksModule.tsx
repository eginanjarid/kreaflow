...
export default function TasksModule({ initialTasks, workspaceId, products = [], contentPillars = '' }: {
  ...
  return (
    <div>
      ...
      {/* Sprint Modal */}
      {sprintModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 300, padding: '20px 20px', overflowY: 'auto' }}>
          <div style={{ background: '#fff', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 20px rgba(0,0,0,0.05)', borderRadius: 20, width: '100%', maxWidth: 580, marginTop: 20, marginBottom: 20 }}>
            ...
            <div style={{ padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
              {sprintError && <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '10px 14px', color: '#dc2626', fontSize: '0.85rem' }}>{sprintError}</div>}
                ...
                {sprintType === 'affiliate' && (
                  <div>
                    ...
                    {products.length > 0 ? (
                      ...
                      <div style={{ background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 8, padding: '10px 14px', fontSize: '0.82rem', color: '#d97706' }}>
                        Belum ada produk. <a href="/catalog" style={{ color: '#d97706', fontWeight: 700 }}>Tambah di Katalog →</a>
                      </div>
                    )}
                  </div>
                )}
