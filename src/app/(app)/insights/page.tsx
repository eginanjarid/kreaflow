  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#f1f5f9', marginBottom: 6 }}>Insights</h1>
        <p style={{ color: '#64748b', fontSize: '0.9rem' }}>Overview performa brand dan konten kamu</p>
      </div>
      ...
      {((todayEntries?.length ?? 0) > 0 || (overdueTasks?.length ?? 0) > 0) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
          {(todayEntries?.length ?? 0) > 0 && (
            <div style={{ background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.25)', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontSize: '1.1rem' }}>📅</span>
              <div>
                <span style={{ color: '#A78BFA', fontWeight: 600, fontSize: '0.875rem' }}>Hari ini ada {todayEntries!.length} konten dijadwalkan</span>
                <span style={{ color: '#64748b', fontSize: '0.8rem' }}> — {todayEntries!.map(e => e.platform).filter((v, i, a) => a.indexOf(v) === i).join(', ')}</span>
              </div>
              <a href="/calendar" style={{ marginLeft: 'auto', color: '#7C3AED', fontSize: '0.78rem', fontWeight: 600, textDecoration: 'none', flexShrink: 0 }}>Lihat Calendar →</a>
            </div>
          )}
          {(overdueTasks?.length ?? 0) > 0 && (
            <div style={{ background: 'rgba(248,113,113,0.07)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <span style={{ color: '#64748b', fontSize: '0.8rem' }}> — segera selesaikan</span>
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 28 }}>
        {statCards.map(s => (
          <div key={s.label} style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 12, padding: '18px 20px' }}>
            <div style={{ fontSize: '0.72rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>{s.label}</div>
      ...
      {isAffiliate && (
        <div style={{ background: 'rgba(52,211,153,0.05)', border: '1px solid rgba(52,211,153,0.2)', borderRadius: 14, padding: '20px', marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: affiliateProducts.length > 0 ? 18 : 0 }}>
            <div style={{ background: '#111', border: '1px solid #1f2e28', borderRadius: 10, padding: '14px 16px' }}>
              <div style={{ fontSize: '0.68rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Produk Affiliate</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#34d399', marginBottom: 4 }}>{affiliateProducts.length}</div>
              <div style={{ fontSize: '0.72rem', color: '#475569' }}>{affiliateProducts.filter(p => p.is_active).length} aktif</div>
            </div>
            <div style={{ background: '#111', border: '1px solid #1f2e28', borderRadius: 10, padding: '14px 16px' }}>
              <div style={{ fontSize: '0.68rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Potensi Komisi</div>
              <div style={{ fontSize: '1.1rem', fontWeight: 700, color: '#34d399', marginBottom: 4 }}>Rp {totalKomisiPotensial.toLocaleString('id-ID')}</div>
              <div style={{ fontSize: '0.72rem', color: '#475569' }}>per transaksi</div>
            </div>
            <div style={{ background: '#111', border: '1px solid #1f2e28', borderRadius: 10, padding: '14px 16px' }}>
              <div style={{ fontSize: '0.68rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Platform</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#34d399', marginBottom: 4 }}>{Object.keys(affiliateByPlatform).length}</div>
              <div style={{ fontSize: '0.72rem', color: '#475569' }}>{Object.keys(affiliateByPlatform).slice(0, 2).join(', ') || '—'}</div>
            </div>
          </div>
          {Object.keys(affiliateByPlatform).length > 0 && (
            <div>
              <div style={{ fontSize: '0.72rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>Breakdown per Platform</div>
          ...
          {affiliateProducts.length === 0 && (
            <p style={{ fontSize: '0.82rem', color: '#475569', margin: 0 }}>Belum ada produk affiliate. Tambahkan di <a href="/catalog" style={{ color: '#34d399', textDecoration: 'none' }}>Catalog →</a></p>
          )}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 12, padding: '20px' }}>
          <div style={{ fontWeight: 600, color: '#e2e8f0', marginBottom: 16, fontSize: '0.9rem' }}>Status Konten di Library</div>
          {contentByStatus.map(({ status, count }) => {
            const total = (ideas || []).length || 1
            const pct = Math.round((count / total) * 100)
            const colors: Record<string, string> = { Draft: '#475569', Ready: '#86efac', Scheduled: '#93c5fd', Posted: '#A78BFA' }
            return (
        ...
        <div style={{ background: '#111', border: '1px solid #2a2a2a', borderRadius: 12, padding: '20px' }}>
          <div style={{ fontWeight: 600, color: '#e2e8f0', marginBottom: 16, fontSize: '0.9rem' }}>Ringkasan Keuangan</div>
          {[
            ...
            { label: 'Saldo Bersih', value: `Rp ${Math.abs(pemasukan - pengeluaran).toLocaleString('id-ID')}`, color: pemasukan >= pengeluaran ? '#A78BFA' : '#f87171' },
          ].map(item => (
            <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #1f1f1f' }}>
          ...
          {(metrics || []).length === 0 && (ideas || []).length === 0 && (
            <p style={{ fontSize: '0.82rem', color: '#475569', marginTop: 12 }}>Mulai isi data di Tracker dan Budget untuk melihat analytics di sini.</p>
          )}
        </div>
      </div>
    </div>
